...
class ToyDiscreteEnv(gym.Env):
    metadata = {'render.modes': ['human', 'rgb_array', 'downsample1', 'downsample2', 'downsample3'], 'video.frames_per_second': 5}
    ...
    def __init__(self, board_size, num_classes, max_pieces, mean_pieces, simple, patch_size=None, picked_info=False):
        ...
        # !!! New Modification. Action 'n+1' = terminate 
        # Cancelled
        self.action_space = spaces.Discrete(self.n) 
        self.observation_space = spaces.Box(low=0, high=1, shape=(num_classes, num_classes, num_classes+1))
    ...
    def _reset(self):
        ...
        self.num_pieces[self.num_pieces < 1] = 1 
        self.prev = None
        self.pos = None
        self.board *= 0
        self.board[:, :, 0] += 1
        ...
        for i in range(self.num_classes):
            ...
            while num_overlaps > 0:
                ...
                num_overlaps = num_piece - np.sum(self.board[:,:,i+1] == 1)
        self.first_choice = x*self.board_size + y
    ...
    def _step(self, action):
        ...
        action += 1
        self.picked_board = np.concatenate((self.board.copy(), np.zeros([self.board_size, self.board_size, 1])), axis=2) 
        ...
        if (self.counter == self.num_pieces).all():
            done = True
            reward += 1000
        ...
        return (self.board.copy(), reward, done, config) 
