...
class ToyContinuousEnv2(gym.Env):
    ...
    def __init__(self, board_size=20, num_classes=2, max_pieces=9, mean_pieces=9, min_pieces=2, patch_size=6, 
        ...
        self.action_space = spaces.Box(low=0, high=1, shape=(k,)) 
    ...
    def _get_top_k(self):
        ...
        top_k = self.np_random.choice(np.arange(size), p=np.ravel(softmax(sal, 0.1)), size=self.k, replace=True)
    ...
    def _step(self, a):
        ...
        a = np.ravel((softmax(a, 1))) 
        action = self.np_random.choice(np.arange(self.k), p=a)
        x = int(self.top_k[action][0])
        y = int(self.top_k[action][1])
